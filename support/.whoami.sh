...
populate_envrc() {
    if [ ! -e "$flag_file" ]; then
        ...
        echo "export username=$USER" | sudo tee -a "$envrc_file"
        ...
        echo "export pwnagotchi=$(if [ "$USER" == "$cwb_username" ] && [ "$computername" == "$cwb_computername" ]; then echo 'Gotcha'; else echo 'Unknown'; fi)" | sudo tee -a "$envrc_file"
        echo "export pwn_installed=0" | sudo tee -a "$envrc_file"
