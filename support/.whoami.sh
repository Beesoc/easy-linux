...
if [ "$computername" != "$(echo $HOST)" ]; then
  printf "${OG}  Warning: $computername does not match $HOST" >&2
fi
if [ "$computername" != "$(cat /etc/hostname)" ]; then
  printf "${OG}  Warning: $computername does not match $(cat /etc/hostname)" >&2
fi
if [ "$computername" != "$(uname -n)" ]; then
  printf "${OG}  Warning: $computername does not match $(uname -n)" >&2
fi
