...
deps_airg_install() {
        ...
        packages=("iw" "procps" "gawk" "xterm" "pciutils" "autoconf" "automake" "libtool" "pkg-config" "libnl-3-dev" "libnl-genl-3-dev" "libssl-dev" "ethtool" "shtool" "rfkill" "zlib1g-dev" "libpcap-dev" "libsqlite3-dev" "libpcre3-dev" "libhwloc-dev" "libcmocka-dev" "hostapd" "wpasupplicant" "tcpdump" "screen" "iw" "usbutils" "groff")
...
if [[ ! -f $HOME/.Xauthority ]] && ! command -v tmux >/dev/null; then
    printf "${OG}  X or Wayland not detected. Installing tmux."
     sudo apt install -y tmux tmux-plugin-manager
     cd $HOME/Downloads/airgeddon/
     AIRGEDDON_WINDOWS_HANDLING=tmux bash airgeddon.sh
     exit 0
else
