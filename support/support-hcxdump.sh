...
choice_func() {
  ...
  printf "  ${CY}On the next screen, you will be asked to pick which wifi adapter to use.\\n"
  printf "  ${CY}Your selected wifi adapter will be unavailable (ex: no internet) while scanning.\\n"
  printf "  ${OG}You need to ${WT}pick Monitor mode ${OG}on the next screen to start hacking.${YW}\\n"
  read -n 1 -r -p "Press any key to continue. " pressany
  source ${scripts_dir}/support/support-monitor.sh
  printf " \\n${WT}"
  printf "${CY}  Now we need to do our initial ${WT}[S]${CY}can for targets or ${WT}[T]${CY}est your wifi adapter's monitor functions.\\n" 
  printf "  ${CY}Press ${WT}[S]${CY}can...Please ${RED}do not interrupt for 40 sec ${CY}scan...${OG}\\n"
  echo
  ...
  if [[ "${torscan}" = "T" ]] || [[ "${torscan}" = "t" ]]; then
    ...
    printf " \\n"
        ...
        if ls "${scripts_dir}/support/misc/hcxdumptool"* >/dev/null 2>&1 || ls "${scripts_dir}/support/hcxdump"* >/dev/null 2>&1; then
            if ls "${scripts_dir}/support/hcxdump"* >/dev/null 2>&1; then
              full_scan
