...
sleep .5  
  ...
  printf "  ${OG}\\n"
  printf "  ${OG}98]${GN} Stuff is broken! Access Troubleshooting Here." 
...
if [[ ${choice} == 1 ]]; then  
    ...
    printf "${YW}      You chose ${WT}Pwnagotchi${YW}. In the Pwnagotchi module, you'll be able to ${WT}backup and restore\\n"
    printf "${YW}      ${YW}your Pwnagotchi. You could also setup a new device, ${WT}upload your collected wifi\\n" 
    printf "${YW}      hashes ${YW}and handshakes or even ${WT}install Pwnagotchi compatible software.${CY}\\n        "
