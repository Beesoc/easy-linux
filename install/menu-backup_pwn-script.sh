...
udev_func() {
    if [[ ! -e /etc/udev/rules.d/99-backup-rule.rules ]]; then
        echo \"ACTION=="add", SUBSYSTEM=="net", "ENV{ID_NET_NAME}"="usb*", ENV"{MAC_ADDRESS}"="${gotcha_mac}|${sniffer_mac}|${FuckThatSh1t_mac}", RUN+="/bin/bash ${scripts_dir}/menu-backup-pwn-script.sh" > sudo tee -a /etc/udev/rules.d/99-backup-rule.rules
